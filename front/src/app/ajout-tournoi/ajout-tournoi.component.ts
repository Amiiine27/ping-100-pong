...
export class AjoutTournoiComponent {
    ...
    constructor(
        private ajoutTournoiService: AjoutTournoiService,
        private http: HttpClient
    ) {}

    onSubmit(tournoiForm: NgForm) {
        if (tournoiForm.valid) {
            this.ajoutTournoiService
                .soumettreTournois(this.tournoiModel)
                .subscribe((response) => {
                    if (!response) {
                        tournoiForm.reset();
            ...
            this.messageError =
                "Le formulaire est invalide. Veuillez vérifier vos données.";
            this.messageSuccess = "";
        }
    }
}
